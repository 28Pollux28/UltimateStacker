...
public class ConfigUtil {
    ...
    public static <T> T getFromConfig(Class<T> configClass, Path path) {
        ...
        try {
            ...
            if (Files.exists(path)) {
                ...
                if (!version.equals(UltimateStackerFabric.VERSION)) {
                    config.getClass().getField("configVersion").set(config, UltimateStackerFabric.VERSION);
    ...
    public static <T> void configToFile(T config){
        Path path = Paths.get("", "config", "ultimatestackerfabric.json");
        try {
            BufferedWriter writer = new BufferedWriter(new FileWriter(path.toFile()));
            writer.write(ConfigUtil.gson.toJson(config));
            writer.close();
